...
test('Multi-User Database Scoping', async (t) => {
  ...
  await t.test('should route Telegram webhook to correct user based on chatId', async () => {
    ...
    await saveSettingsForUser('thanhhuong', { ...backupThanhHuong, telegramBotToken: 'mock-token', telegramChatId: 'CHAT_ID_REVOKED', allowedChatIdsStr: 'CHAT_ID_REVOKED, 10001', telegramSessionState: { userState: 'IDLE' }, userNotes: [] });
